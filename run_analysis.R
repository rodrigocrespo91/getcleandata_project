...
# Second tidy data
activity<-c(rep("Walking",6),rep("Walking upstairs",6),rep("Walking downstairs",6),
            rep("Sitting",6),rep("Standing",6),rep("Laying",6))
subject <- c(rep(c(1,2,3,4,5,6),6))
data2 <- data.frame(activity,subject)
y <- vector("numeric",length=18)
